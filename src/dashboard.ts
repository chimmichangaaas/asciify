...
document.querySelectorAll<HTMLButtonElement>('.share-social').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    switch (net) {
      ...
      case 'reddit':   target = `https://reddit.com/submit?url=${url}&title=${encodeURIComponent('Asciify — typographic art tool')}`; break;
...
# Generated by Asciify — ${new Date().toISOString().slice(0, 10)}
...
btnCpp.addEventListener('click', () => {
  ...
  const cpp = `// Generated by Asciify — ${new Date().toISOString().slice(0, 10)}
// Size: ${f.columns} × ${f.rows} chars
...
btnJs.addEventListener('click', () => {
  ...
  const js = `// Generated by Asciify — ${new Date().toISOString().slice(0, 10)}
    ...
    if (ENDPOINT) {
      try {
        await fetch(ENDPOINT, {
          ...
          body: JSON.stringify({ email, source: 'asciify', ts: Date.now() }),
        });
      } catch {}
    }
    // Visual confirmation
